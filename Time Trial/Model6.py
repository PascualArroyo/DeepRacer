def reward_function(params):
 ...
 #Three waypoint forward
 forward2_waypoint = closest_waypoints[1]+3
 if forward2_waypoint >= tam:
     forward2_waypoint = 2

 # Points of track
 forward_point = waypoints[forward_waypoint]
 forward2_point = waypoints[forward2_waypoint]
 ...
 ABS_STEERING_THRESHOLD_1 = 10
 ABS_STEERING_THRESHOLD_2 = 20
 

 if abs(track_direction-track_direction2) < 2:
     # Penalize reward if the agent is steering too much
     if steering > ABS_STEERING_THRESHOLD_2:
         reward *= 0.2
     elif steering > ABS_STEERING_THRESHOLD_1:
         reward *= 0.8
     
 elif abs(track_direction-track_direction2) < 8:
     # Penalize reward if the agent is steering too much
      if steering > ABS_STEERING_THRESHOLD_2:
          reward *= 0.6
      elif steering > ABS_STEERING_THRESHOLD_1:
 ...
 #4 Speed
 track_direction3 = math.atan2(forward2_point[1] - prev_point[1], forward2_point[0] - prev_point[0])
 # Convert to degree
 track_direction3 = math.degrees(track_direction3)
        
        
 if abs(track_direction-track_direction3) < 2:
     if speed < 0.8:
         reward *= 0.1
     elif speed < 1.5:
         reward *= 0.5
 ...
 elif abs(track_direction-track_direction3) < 8:
     if speed < 0.8:
         reward *= 0.5
     elif speed < 1.5:
         reward *= 1
 ...
 #5 Optimize track
 TOTAL_NUM_STEPS1 = 500
 TOTAL_NUM_STEPS2 = 600
 TOTAL_NUM_STEPS3 = 700
